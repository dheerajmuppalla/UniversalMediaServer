...
public class PlayerApiServlet extends GuiHttpServlet {
			...
			switch (path) {
				...
				case "/findMetadata" -> {
					if (!renderer.havePermission(Permissions.WEB_PLAYER_EDIT)) {
						...
						String lang = action.has("lang") && !action.get("lang").isJsonNull()
								? action.get("lang").getAsString()
								: null;
				...
				case "/setMetadata" -> {
					if (!renderer.havePermission(Permissions.WEB_PLAYER_EDIT)) {
						...
						String mediaType = action.get("media_type").getAsString();
						// Long episode = action.has("episode") && !action.get("episode").isJsonNull() ?
						// action.get("episode").getAsLong() : null;
						// Long season = action.has("season") && !action.get("season").isJsonNull() ?
						// action.get("season").getAsLong() : null;
	...
	private static String getIconNameFromFolderName(String folderName, String defaultIconName) {
		...
		// Some of these are dynamic folders we supply, others are guesses at common
		// folder names users will have
	...
	private static JsonObject getBrowsePage(WebGuiRenderer renderer, String id, String search, String lang)
			...
			if (resource.isFolder()) {
				...
				if (resource.getParent().getSystemName().equals("TvShows") ||
						...
						(resource.getParent().getParent() != null &&
								...
								resource.getParent().getParent().getParent().getSystemName()
										.equals("FilterByInformation"))) {
					isDisplayFoldersAsThumbnails = true;
				}

				if (!isDisplayFoldersAsThumbnails
						|| !(isDisplayFoldersAsThumbnails && resource instanceof MediaLibraryFolder)) {
					...
					if (id.equals("0") && resource.getSystemName().equals("MediaLibrary")) {
						List<StoreResource> mediaLibraryChildren = renderer.getMediaStore()
								.getResources(resource.getId(), true);
						JsonObject mediaLibraryFolder;
						StoreResource videoFolder = UMSUtils.getFirstResourceWithSystemName(mediaLibraryChildren,
								"Video");
						if (videoFolder != null) {
						...
						StoreResource audioFolder = UMSUtils.getFirstResourceWithSystemName(mediaLibraryChildren,
								"Audio");
						if (audioFolder != null) {
							mediaLibraryFolder = new JsonObject();
						...
						StoreResource imagesFolder = UMSUtils.getFirstResourceWithSystemName(mediaLibraryChildren,
								"Photo");
						...
						if (videoFolder != null) {
							...
							jMediasSelections.add("recentlyAdded",
									getMediaLibraryFolderChilds(videoFolder, renderer, "RecentlyAdded", lang));
							jMediasSelections.add("recentlyPlayed",
									getMediaLibraryFolderChilds(videoFolder, renderer, "RecentlyPlayed", lang));
							jMediasSelections.add("inProgress",
									getMediaLibraryFolderChilds(videoFolder, renderer, "InProgress", lang));
							jMediasSelections.add("mostPlayed",
									getMediaLibraryFolderChilds(videoFolder, renderer, "MostPlayed", lang));
		...
		if (rootResource instanceof MediaLibraryFolder folder) {
			if (folder.isTVSeries()) {
				...
				if (metadata != null) {
					metadata.addProperty("isEditable",
							renderer.havePermission(Permissions.WEB_PLAYER_EDIT) && TMDB.isReady());
			...
			// Check whether this resource is expected to contain folders that display as
			// big thumbnails
		...
		result.addProperty("name", id.equals("0") || rootResource == null ? CONFIGURATION.getServerDisplayName()
				: rootResource.getLocalizedDisplayName(lang));
	...
	private static JsonObject getShowPage(WebGuiRenderer renderer, String id, String lang)
			throws IOException, InterruptedException {
	...
	private static JsonObject getPlayPage(WebGuiRenderer renderer, String id, String lang)
			throws IOException, InterruptedException {
		...
		if (isVideo) {
			...
			if (item.getMediaStatus() != null && item.getMediaStatus().getLastPlaybackPosition() != null
					&& item.getMediaStatus().getLastPlaybackPosition() > 0) {
		...
		media.addProperty("isDownload",
				renderer.havePermission(Permissions.WEB_PLAYER_DOWNLOAD) && CONFIGURATION.useWebPlayerDownload());
	...
	private static JsonObject getEditData(WebGuiRenderer renderer, String id) throws IOException, InterruptedException {
		...
		StoreResource resource = renderer.getMediaStore().getResource(id);
		StoreItem item = resource instanceof StoreItem libraryItem ? libraryItem : null;
		MediaLibraryTvSeries tvSeries = resource instanceof MediaLibraryTvSeries mediaLibraryTvSeries
				? mediaLibraryTvSeries
				: null;
	...
	private static JsonObject getMediaInfo(WebGuiRenderer renderer, String id)
			throws IOException, InterruptedException {
	...
	private static JsonArray getMetadataResults(WebGuiRenderer renderer, String id, String mediaType, String search,
			Integer year, String lang) throws IOException, InterruptedException {
	...
	private static boolean setMetadata(WebGuiRenderer renderer, String id, Long tmdbId, String mediaType)
			throws IOException, InterruptedException {
		...
		} else if (mediaType.equals("tv") && tvSeries != null && tvSeries.getTvSeriesMetadata() != null
				&& tvSeries.getTvSeriesMetadata().getTvSeriesId() != null) {
			return TMDB.updateTvShowMetadata(tvSeries.getTvSeriesMetadata().getTvSeriesId(), tmdbId);
		}
		return false;
	}

	private static boolean setFullyPlayed(WebGuiRenderer renderer, String id, boolean fullyPlayed)
			throws IOException, InterruptedException {
		StoreResource resource = renderer.getMediaStore().getResource(id);
	...
	private static WebGuiRenderer getValidRendererOrRespondError(HttpServletRequest req, HttpServletResponse resp,
			String[] data, int count, int permissions) throws IOException {
	...
	private static void sendRawMedia(HttpServletRequest req, HttpServletResponse resp, boolean isDownload)
			throws IOException {
		String path = req.getPathInfo();
		...
		try {
			...
			if (isDownload) {
				resp.setHeader("Content-Disposition",
						"attachment; filename=\"" + new File(item.getFileName()).getName() + "\"");
			}
			if (in != null) {
				if (in.available() != len) {
	...
	private static void sendDownloadMedia(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		String path = req.getPathInfo();
		String[] pathData = path.split("/");
		WebGuiRenderer renderer = getValidRendererOrRespondError(req, resp, pathData, 4,
				Permissions.WEB_PLAYER_DOWNLOAD);
		if (renderer == null) {
			return;
		}
		String id = pathData[3];
	...
	private static void sendImageMedia(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		try {
			...
			long len;
			ByteRange range;
			if (item.getMediaInfo() != null && item.getMediaInfo().isImage()
					&& item.getMediaInfo().getImageInfo() != null) {
				ImageInfo imageInfo = item.getMediaInfo().getImageInfo();
				boolean supported = renderer.isImageFormatSupported(imageInfo.getFormat());
				mime = item.getFormat() != null ? item.getFormat().mimeType() : item.getMimeType();
				...
				if (supported) {
					...
					if (item.isTranscoded() && item.getTranscodingSettings().getEngine() instanceof ImageEngine) {
						ProcessWrapper transcodeProcess = item.getTranscodingSettings().getEngine().launchTranscode(
								item,
								item.getMediaInfo(),
					...
					Image image = Image.toImage(imageInputStream, 3840, 2400, ImagesUtil.ScaleType.MAX,
	...
	private static void sendMedia(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		try {
			// hls part
			if (item.getFormat().isVideo() && item.isTranscoded()
					&& item.getTranscodingSettings().getEngine() instanceof FFmpegHlsVideo) {
				resp.setHeader("Server", MediaServer.getServerName());
				if (uri.endsWith("/chapters.vtt")) {
					...
					if (pathData[5].endsWith(".m3u8")) {
						...
						String response = HlsHelper.getHLSm3u8ForRendition(item, renderer,
								req.getServletPath() + "/media/" + sessionId + "/", rendition);
					...
					String response = HlsHelper.getHLSm3u8(item, renderer,
							req.getServletPath() + "/media/" + sessionId + "/");
					respond(req, resp, response, 200, HTTPResource.HLS_TYPEMIME);
				}
			} else {
				AsyncContext async = req.startAsync();
	...
	private static void sendThumbnail(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if (thumb != null) {
			AsyncContext async = req.startAsync();
			resp.setContentType(
					ImageFormat.PNG.equals(thumb.getFormat()) ? HTTPResource.PNG_TYPEMIME : HTTPResource.JPEG_TYPEMIME);
			resp.setHeader("Server", MediaServer.getServerName());
			resp.setHeader("Accept-Ranges", "bytes");
			resp.setHeader("Connection", "keep-alive");
			resp.setStatus(200);
	...
	private static JsonObject getMetadataAsJsonObject(StoreResource resource, Renderer renderer, String lang) {
		...
		if (CONFIGURATION.isShowMediaLibraryFolder()) {
			...
			StoreResource mediaLibraryFolder = UMSUtils.getFirstResourceWithSystemName(rootFolderChildren,
					"MediaLibrary");
			if (mediaLibraryFolder == null) {
				return null;
			}
			List<StoreResource> mediaLibraryChildren = renderer.getMediaStore().getResources(mediaLibraryFolder.getId(),
					true);
			StoreResource videoFolder = UMSUtils.getFirstResourceWithSystemName(mediaLibraryChildren, "Video");

			boolean isRelatedToTV = resource instanceof MediaLibraryTvSeries
					|| resource instanceof MediaLibraryTvEpisode;
			String folderName = isRelatedToTV ? "TvShows" : "Movies";
			List<StoreResource> videoFolderChildren = renderer.getMediaStore().getResources(videoFolder.getId(), true);
			StoreResource tvShowsOrMoviesFolder = UMSUtils.getFirstResourceWithSystemName(videoFolderChildren,
					folderName);

			List<StoreResource> tvShowsOrMoviesChildren = renderer.getMediaStore()
					.getResources(tvShowsOrMoviesFolder.getId(), true);
			StoreResource filterByInformationFolder = UMSUtils.getFirstResourceWithSystemName(tvShowsOrMoviesChildren,
					"FilterByInformation");

			if (filterByInformationFolder != null) {
				List<StoreResource> filterByInformationChildren = renderer.getMediaStore()
						.getResources(filterByInformationFolder.getId(), true);

				for (int filterByInformationChildrenIterator = 0; filterByInformationChildrenIterator < filterByInformationChildren
						.size(); filterByInformationChildrenIterator++) {
					StoreResource filterByInformationChild = filterByInformationChildren
							.get(filterByInformationChildrenIterator);
	...
	private static void addJsonArrayStoreIds(final JsonObject object, final String memberName,
			final StoreResource folder, final Renderer renderer) {
				...
				if (!array.isEmpty()) {
					...
					for (JsonElement child : array) {
						if (child.isJsonPrimitive()) {
							...
							if (folder != null) {
								List<StoreResource> folderChildren = renderer.getMediaStore()
										.getResources(folder.getId(), true);
	...
	private static void addStringStoreId(final JsonObject object, final String memberName, final StoreResource folder,
